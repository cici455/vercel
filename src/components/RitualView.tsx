  ...
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none z-0">
      ...
      <div className="absolute inset-0 bg-[#050505]">
        {/* Star Dust */}
        <div className="absolute inset-0 opacity-30 bg-[url('https://grainy-gradients.vercel.app/noise.svg')] mix-blend-overlay z-10"></div>
      </div>

      {/* Giant Rotating Planet */}
      <motion.img 
        src="/planet.png"
        alt="Cosmic Planet"
        className="fixed -bottom-[80vw] left-1/2 -translate-x-1/2 w-[140vw] max-w-none opacity-80 z-0"
        animate={{ rotate: 360 }}
        transition={{ duration: 120, repeat: Infinity, ease: "linear" }}
      />

      {/* The Rift (Gate) */}
      <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-1 h-40 flex justify-center items-end z-20">
        <div className="relative w-1 h-full bg-white shadow-[0_0_40px_cyan] drop-shadow-[0_0_10px_rgba(255,255,255,0.8)] animate-pulse"></div>
      </div>
    </div>
  );
};
  ...
  return (
    <motion.div
      ...
      className={`absolute w-[280px] h-[420px] rounded-xl border backdrop-blur-md overflow-hidden cursor-pointer transition-all duration-500
        ${isActive 
          ? `z-20 scale-110 opacity-100 ${theme.color} bg-black/40 shadow-lg` 
          : `z-10 scale-90 opacity-50 border-white/10 bg-black/60 blur-sm grayscale-[0.8]`
        }
      `}
      initial={false}
      animate={{
    ...
    >
      ...
      <div className="relative h-full flex flex-col items-center justify-between p-6">
        ...
        <div className="relative flex-1 flex items-center justify-center w-full">
          <div className="w-40 h-40 rounded-full bg-gradient-to-tr from-cyan-900/40 to-purple-900/40 border border-white/10 flex items-center justify-center">
             <div className="w-32 h-32 rounded-full border border-dashed border-white/20 animate-spin-slow"></div>
          </div>
        </div>
...
const RitualView: React.FC<RitualViewProps> = ({ onOpenGate, onBack }) => {
  ...
  return (
    <div className="fixed inset-0 w-full h-full bg-[#050505] overflow-hidden flex flex-col items-center">
      ...
      <div className="relative flex-1 w-full flex items-center justify-center perspective-[1000px] z-10">
        {/* Navigation Buttons */}
      ...
      <div className="absolute bottom-10 z-50 w-full max-w-lg px-6 flex flex-col items-center gap-6">
        ...
        <div className="w-full relative group">
          <div className="relative flex items-center bg-black/30 backdrop-blur-md border border-white/10 rounded-full p-2 transition-all focus-within:border-white/20 focus-within:bg-black/50">
            <input 
              type="text" 
        ...
        >
          <div className="relative flex items-center gap-3">
            <span className="font-cinzel font-bold text-white tracking-[0.2em] text-lg group-hover:text-cyan-200 transition-colors drop-shadow-[0_0_10px_rgba(255,255,255,0.8)]">
              OPEN THE GATE
            </span>
          </div>
        </button>
